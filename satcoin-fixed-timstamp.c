...
int verifyhash(unsigned int *block)
{
  ...
  *u_nonce = nondet_uint();
  unsigned int start_nonce = nondet_uint();
  unsigned int end_nonce = nondet_uint();
  *u_timestamp = nondet_uint();
...
__CPROVER_assume(*u_nonce > nonce_start && *u_nonce < nonce_end); // used nonce should stay in the given range
  ...
  /* =============================== GENESIS BLOCK ============================================= */
   __CPROVER_assume(start_nonce==4043570729); // 1 nonces only
   __CPROVER_assume(end_nonce==4043570732); // 1 nonces only
   __CPROVER_assume(*u_nonce > start_nonce && *u_nonce <= end_nonce);
   __CPROVER_assume(*u_timestamp==2980447515); // 1 nonces only
...
unsigned int input_blocks[20] = {
 ...
 // time, bits, nonce
 2004092496, 29804475134,// 1
 4043570731
}; 

unsigned int input_block[20] ={0};
